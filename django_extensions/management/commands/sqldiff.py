...
class SQLDiff(object):
    ...
    SQL_FIELD_MISSING_IN_DB = lambda self, style, qn, args: "%s %s\n\t%s %s %s;" % (style.SQL_KEYWORD('ALTER TABLE'), style.SQL_TABLE(
        qn(args[0])), style.SQL_KEYWORD('ADD'), style.SQL_FIELD(qn(args[1])), style.SQL_COLTYPE(args[2]))
    SQL_FIELD_MISSING_IN_MODEL = lambda self, style, qn, args: "%s %s\n\t%s %s;" % (style.SQL_KEYWORD(
        'ALTER TABLE'), style.SQL_TABLE(qn(args[0])), style.SQL_KEYWORD('DROP COLUMN'), style.SQL_FIELD(qn(args[1])))
    ...
    SQL_UNIQUE_MISSING_IN_DB = lambda self, style, qn, args: "%s %s\n\t%s %s (%s);" % (style.SQL_KEYWORD(
        'ALTER TABLE'), style.SQL_TABLE(qn(args[0])), style.SQL_KEYWORD('ADD'), style.SQL_KEYWORD('UNIQUE'), style.SQL_FIELD(qn(args[1])))
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if '.' in reverse_type:
            ...
            field_db_type = getattr(module, package_name)(
                **kwargs).db_type(connection=connection)
        else:
            field_db_type = getattr(models, reverse_type)(
                **kwargs).db_type(connection=connection)
    ...
    def find_unique_missing_in_db(self, meta, table_indexes, table_name):
        for field in all_local_fields(meta):
            if field.unique:
                ...
                if attname in table_indexes and table_indexes[
                        attname]['unique']:
                    continue
                self.add_difference(
                    'unique-missing-in-db', table_name, attname)
    ...
    def find_unique_missing_in_model(self, meta, table_indexes, table_name):
        ...
        for att_name, att_opts in table_indexes.iteritems():
            if att_opts['unique'] and att_name in fields and not fields[
                    att_name]:
                if att_name in flatten(meta.unique_together):
                    continue
                self.add_difference(
                    'unique-missing-in-model', table_name, att_name)
        ...
        for field in all_local_fields(meta):
            if field.db_index:
                ...
                if attname not in table_indexes:
                    self.add_difference(
                        'index-missing-in-db', table_name, attname)
        ...
        for att_name, att_opts in table_indexes.iteritems():
            if att_name in fields:
                ...
                if att_opts['unique'] and field.unique:
                    continue
                if att_opts['unique'] and att_name in flatten(
                        meta.unique_together):
                    continue
                self.add_difference(
                    'index-missing-in-model', table_name, att_name)
        ...
        for app_label, model_name, diffs in self.differences:
            ...
            for diff in diffs:
                ...
                text = self.DIFF_TEXTS[diff_type] % dict(
                    (str(i), style.SQL_TABLE(e)) for i, e in enumerate(diff_args))
                text = "'".join(
                    i %
    ...
    def print_diff_sql(self, style):
        ...
        has_differences = max(
            [len(diffs) for app_label, model_name, diffs in self.differences])
...
class MySQLDiff(SQLDiff):
    ...
    def get_field_db_type(self, description, field=None, table_name=None):
        ...
        if description[1] not in [FIELD_TYPE.TINY,
                                  FIELD_TYPE.SHORT]:  # exclude tinyints from conversion.
        ...
        if field:
            if field.primary_key and (
                    db_type == 'integer' or db_type == 'bigint'):
            ...
            # nothing else.
            if db_type == 'integer' and description[
                    1] == FIELD_TYPE.TINY and description[4] == 1:
                db_type = 'bool'
        ...
        if field:
            ...
            if table_name:
                ...
                check_constraint = self.check_constraints.get(
                    (tablespace, table_name, field.attname), {}).get(
                ...
                null = self.null.get(
                    (tablespace, table_name, field.attname), 'fixme')
                if field.null != null:
                    action = field.null and 'DROP' or 'SET'
                    self.add_difference(
                        'notnull-differ', table_name, field.name, action)
        return db_type
