...
def render_field(
    ...
    """
    FAIL_SILENTLY = getattr(settings, 'CRISPY_FAIL_SILENTLY', False)
    ...
    if hasattr(field, 'render'):
        ...
        # This allows fields to be unicode strings, always they don't use non
        # ASCII
        try:
            if isinstance(field, unicode):
                field = str(field)
            # If `field` is not unicode then we turn it into a unicode string, otherwise doing
            # str(field) would give no error and the field would not be
            # resolved, causing confusion
            else:
    ...
    try:
        # Injecting HTML attributes into field's widget, Django handles
        # rendering these
            ...
            for index, (widget, attr) in enumerate(zip(widgets, list_attrs)):
                if hasattr(field_instance.widget, 'widgets'):
                    if 'type' in attr and attr['type'] == "hidden":
                        ...
                        field_instance.widget.widgets[
                            index] = field_instance.hidden_widget()
    ...
    if field_instance is None:
        ...
        # We save the Layout object's bound fields in the layout object's
        # `bound_fields` list
        if layout_object is not None:
            if hasattr(
                    layout_object,
                    'bound_fields') and isinstance(
...
def flatatt(attrs):
    ...
    """
    return u''.join([u' %s="%s"' % (k.replace('_', '-'),
                                    conditional_escape(v)) for k, v in attrs.items()])
