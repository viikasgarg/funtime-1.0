...
def NameVoweltoNumber(name, WithMasters):
    ...
    if name.isalpha():
        ...
        for char in list(name):
            if char in ['A', 'E', 'I', 'O', 'U'] or (
                    prevCharIsVowel and char in ['Y', 'W']):
                numChar = ord(char) - 64
...
def testPinnacleNumbers():
    ...
    if pnum['first_pinnacle'] != 1 and pnum['second_pinnacle'] != 3 and pnum[
            'third_pinnacle'] != 4 and pnum['fourth_pinnacle'] != 5:
        assert False
########################################################################
...
def NumKundliInfo(birthday, first_name='', middle_name='', last_name=''):
    ...
    year = birthday.year
    info['lifepath'] = get_data(
        'LifePath', 'LifePath', LifePathValue(
            day, month, year))
    info['exp_num'] = get_data(
        'ExpNum', 'ExpNum', ExpressionValue(
            first_name, middle_name, last_name))
    info['soul_urge'] = get_data(
        'SoulUrge', 'SoulUrge', SoulUrgeValue(
            first_name, middle_name, last_name))
    info['personal_nums'] = get_data(
        'PersonalYear',
        'PersonalYear',
        PersonalYearValue(
            day,
    ...
    first_pinnacle = get_data(
        'Pinnacle/com',
        'pinnacle',
        pinnacle_nums['first_pinnacle']) + get_data(
        'Pinnacle/first',
        'pinnacle',
        pinnacle_nums['first_pinnacle'])

    second_pinnacle = get_data(
        'Pinnacle/com',
        'pinnacle',
        pinnacle_nums['second_pinnacle']) + get_data(
        'Pinnacle/sec_third',
        'pinnacle',
        pinnacle_nums['second_pinnacle'])

    third_pinnacle = get_data(
        'Pinnacle/com',
        'pinnacle',
        pinnacle_nums['third_pinnacle']) + get_data(
        'Pinnacle/sec_third',
        'pinnacle',
        pinnacle_nums['third_pinnacle'])

    fourth_pinnacle = get_data(
        'Pinnacle/com',
        'pinnacle',
        pinnacle_nums['fourth_pinnacle']) + get_data(
        'Pinnacle/final',
        'pinnacle',
        pinnacle_nums['fourth_pinnacle'])
